...
FOR SELECT 
USING (
  has_permission(auth.uid(), 'employees.read'::app_permission) OR
...
FOR INSERT 
WITH CHECK (
  has_permission(auth.uid(), 'employees.create'::app_permission) OR
...
FOR UPDATE 
USING (
  has_permission(auth.uid(), 'employees.update'::app_permission) OR
