...
BEGIN
  ...
  -- Perform the update using quoted column names
...
-- Re-create get_employee_sensitive_data with proper search_path
...
BEGIN
  ...
  IF NOT (public.has_permission(auth.uid(), 'employees.read'::public.app_permission) OR 
          public.has_role(auth.uid(), 'super_admin'::public.app_role) OR 
          public.has_role(auth.uid(), 'admin'::public.app_role)) THEN
