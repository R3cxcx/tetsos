-- Add deletability tracking columns to employees table
ALTER TABLE public.employees 
ADD COLUMN is_deletable boolean NOT NULL DEFAULT true,
ADD COLUMN deleted_at timestamp with time zone DEFAULT NULL;

-- Update employees RLS DELETE policy to check deletability
...
-- Create safe delete employee function
...
BEGIN
  ...
  -- Check if marked as non-deletable
  IF v_employee.is_deletable = false THEN
    RETURN jsonb_build_object(
      'success', false,
      'message', 'Employee cannot be deleted due to existing transactions'
    );
  END IF;
  ...
  WHERE hired_employee_id = p_employee_id;
  ...
  -- If there are references, mark as non-deletable and return error
  IF v_total_references > 0 THEN
    UPDATE public.employees 
    SET is_deletable = false, updated_at = now()
    WHERE id = p_employee_id;

    RETURN jsonb_build_object(
      'success', false,
      'message', format('Employee cannot be deleted. Found %s related records (Attendance: %s, Recruitment: %s, Hiring: %s, Mappings: %s)', 
        v_total_references, v_attendance_count, v_recruitment_count, v_hiring_count, v_mapping_count),
  ...
  -- Safe to delete - perform actual deletion
...
-- Create triggers to mark employees as non-deletable when referenced
...
BEGIN
  ...
  -- Handle recruitment_requests
  IF TG_TABLE_NAME = 'recruitment_requests' AND NEW.hired_employee_id IS NOT NULL THEN
    v_employee_uuid := NEW.hired_employee_id;
  END IF;

  -- Handle hiring_requests
  IF TG_TABLE_NAME = 'hiring_requests' AND NEW.hired_employee_id IS NOT NULL THEN
    v_employee_uuid := NEW.hired_employee_id;
  END IF;

  -- Handle user_id_mapping
  IF TG_TABLE_NAME = 'user_id_mapping' THEN
    SELECT e.id INTO v_employee_uuid
    FROM public.employees e
    WHERE e.employee_id = NEW.employee_id;
  END IF;

  -- Update employee deletability if found
...
-- Update get_employees_basic_data function to include is_deletable
